...
function textURL(data) {
	let unformatMessage = '';
	data['text'].forEach(function(item, index){
		
		unformatMessage = unformatMessage + item + "<br>";
		}
	);

	$("div#unchange").html(unformatMessage);
};

function getData() {
	$.getJSON(dataURL, textURL);
};

function readText() {
	$("#first-button").click(getData);
};
...
function pushText() {
	$.getJSON(dataURL, formatted);
};

function changeText() {
	$("#second-button").click(pushText);
};

$(document).ready(readText);
